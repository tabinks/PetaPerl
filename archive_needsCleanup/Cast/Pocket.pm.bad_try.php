...
###########################################################################    
    ...
    if (-e "$file") {
	open (POC, "< $file") 
	    or error("Cast::Pocket::pocket_coordinates:\tCouldn't open $file: $!");
    } else {
	error("Cast::Pocket::pocket_coordinates:\tCouldn't open $file: $!");
    }
	...
	next if $_ =~ /^\#/;
	next if $_ =~ /^\s+$/;
	%tmp =  &Cast::Common::_atomRecord($_) ;
	print $tmp{record_name};
#	    push (@return, &Cast::Common::_atomRecord($_) );
    }	
    close POC;     

#    for $i (0..$#return) {
#	for $role (keys %{ $return[$i]} ) {
#	    print "$i $role $return[$i]{$role}\n";
#	}
#    }
    # Return to program
    return @return
...
###########################################################################    
    ...
    print $self->{PDB_ARRAY};

    foreach ($self->{PDB_ARRAY}) {
